...
export default function HeroBoard() {
  return (
    <div style={{ position: 'relative', width: '100%', backgroundColor: '#E8501A', userSelect: 'none' }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Caveat:wght@500;700&family=Archivo+Black&display=swap');`}</style>

      {/* MOBILE LAYOUT */}
      <div className="flex flex-col items-center justify-center py-16 px-6 gap-5 md:hidden">
        <p style={{ fontFamily: 'Caveat,cursive', color: 'rgba(255,255,255,0.88)', fontSize: 20, margin: 0 }}>Hey! I'm Krishna</p>
        <h1 style={{ fontFamily: "'Archivo Black',sans-serif", fontSize: 'clamp(42px,12vw,72px)', color: 'white', lineHeight: 1, textAlign: 'center', textTransform: 'uppercase', margin: 0 }}>
          UI/UX<br />DESIGNER
        </h1>
        <p style={{ fontFamily: 'Caveat,cursive', color: 'rgba(255,255,255,0.82)', fontSize: 16, textAlign: 'center', maxWidth: 300, lineHeight: 1.5, margin: 0 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center', marginTop: 8 }}>
          ...
          <Sticky rotate={1} color="#FFE082">{'Color palettes >\ntherapy\nsometimes'}</Sticky>
        </div>
        <div style={{ background: 'white', padding: '8px 8px 24px 8px', boxShadow: '4px 6px 22px rgba(0,0,0,0.28)', transform: 'rotate(-2deg)', width: 110 }}>
          <img src={meImg} alt="Krishna" style={{ width: '100%', height: 100, objectFit: 'cover', display: 'block' }} />
        </div>
      </div>

      {/* DESKTOP LAYOUT */}
      <div className="hidden md:block" style={{ position: 'relative', height: '85vh' }}>
